...
! Period parameters
    integer, parameter :: parN = 624, parN1 = parN + 1

! the array for the state vector
    integer, save, dimension(0:parN-1) :: mt
    integer, save                   :: mti = parN1
...
!Initialization subroutine
  subroutine sgrnd(seed)
    implicit None
!
!      setting initial seeds to mt[parN] using
...
!                        mag01(x) = x * MATA for x=0,1
    ...
    if(mti.ge.parN) then
!                       generate parN words at one time
      if(mti.eq.parN+1) then
...
!                              a default initial seed is used
      ...
      do kk=parN-M,parN-2
          y=ior(iand(mt(kk),UMASK),iand(mt(kk+1),LMASK))
          mt(kk)=ieor(ieor(mt(kk+(M-parN)),ishft(y,-1)),mag01(iand(y,1)))
      enddo
      y=ior(iand(mt(parN-1),UMASK),iand(mt(0),LMASK))
      mt(parN-1)=ieor(ieor(mt(M-1),ishft(y,-1)),mag01(iand(y,1)))
      mti = 0
    endif
